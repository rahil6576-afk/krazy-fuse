...
class CaveGravityRunner {
    ...
    startGame() {
        ...
        this.flips = 0;
        this.level = 1;
        this.baseSpeed = 3.6; // Accessible, smooth cavern speed
        this.speed = 3.6;
        this.combo = 1;
        this.highestCombo = 1;
    ...
    die(reason) {
        ...
        this.shakeTimer = 32;
        this.createRockExplosion(this.player.x + 14, this.player.y + 21, '#ef4444');

        if (this.distance > this.highScore) {
            this.highScore = Math.floor(this.distance);
            localStorage.setItem('gravity_highscore', this.highScore);
        }
        this.totalCoins += this.coins;
        localStorage.setItem('gravity_total_coins', this.totalCoins);
    ...
    update() {
        ...
        // Level Up check every 750 meters
        const targetLevel = Math.min(5, Math.floor(this.distance / 750) + 1);
        ...
        this.player.runCycle += this.speed * 0.08;

        // Gravity Physics
        const gravAcc = this.player.gravDir * 1.25;
        ...
        if (this.player.gravDir === 1 && this.player.y + this.player.h >= this.floorY) {
            ...
            this.player.angle += this.player.gravDir * 0.18;
        }
        ...
        if (this.spawnTimer <= 0) {
            ...
            this.spawnTimer = Math.max(38, Math.floor(92 - this.level * 8));
        }
        ...
        for (let i = this.obstacles.length - 1; i >= 0; i--) {
            ...
            // Swinging Crystal Pendulum Trap
            if (obs.type === 'PENDULUM') {
                obs.swingAngle += 0.05;
                obs.y = 300 + Math.sin(obs.swingAngle) * 110;
            }

            // Near-miss detection
            if (!obs.nearMissed && Math.abs(this.player.x - obs.x) < 22) {
                const distY = Math.abs((this.player.y + 21) - (obs.y + obs.h/2));
                if (distY < 50 && distY > 22) {
                    obs.nearMissed = true;
                    this.combo = Math.min(10, this.combo + 1);
                    if (this.combo > this.highestCombo) this.highestCombo = this.combo;
                    this.addFloatingText(`NEAR MISS! ${this.combo}x`, this.player.x + 40, this.player.y, '#ffd166', 15);
                }
            }

            // Collision Check
            if (this.checkCollision(this.player, obs)) {
                this.die('Crushed by cave stalactite');
                return;
            }

            if (obs.x + obs.w < -60) this.obstacles.splice(i, 1);
        }

        // Update Collectibles (Gold Nuggets, Amethyst, Rubies, Relics)
        for (let i = this.collectibles.length - 1; i >= 0; i--) {
            ...
            if (this.player.magnetTimer > 0) {
                const dx = (this.player.x + 14) - col.x;
                const dy = (this.player.y + 21) - col.y;
            ...
            if (this.checkCollision(this.player, col)) {
                if (col.type === 'GEM') {
                    this.coins += 1 * this.combo;
                    this.distance += 12;
                    this.createGemSparkles(col.x, col.y);
                    window.gravityAudio.playCoin();
                } else if (col.type === 'SHIELD') {
                    this.player.shield = true;
                    this.addFloatingText('CRYSTAL SHIELD ACTIVE!', this.player.x, this.player.y - 20, '#38bdf8', 16);
                    window.gravityAudio.playPowerup();
                } else if (col.type === 'MAGNET') {
                    this.player.magnetTimer = 360;
                    this.addFloatingText('RELIC MAGNET ACTIVE!', this.player.x, this.player.y - 20, '#ffd166', 16);
                    window.gravityAudio.playPowerup();
                } else if (col.type === 'SLOWMO') {
                    this.player.slowMoTimer = 240;
                    this.addFloatingText('CHRONO AMBER SLOW-MO!', this.player.x, this.player.y - 20, '#f97316', 16);
                    window.gravityAudio.playPowerup();
                }
            ...
            if (col.x < -60) this.collectibles.splice(i, 1);
        }
    ...
    spawnProceduralCavePattern() {
        ...
        if (rand < 0.32) {
            // Floor Stalagmite cluster (1 to 2 rocky spikes)
            const count = Math.random() < 0.5 ? 1 : 2;
            for (let i = 0; i < count; i++) {
                const height = Math.floor(Math.random() * 30) + 45;
                this.obstacles.push({
                    type: 'STALAGMITE',
                    x: startX + i * 36,
            ...
            this.spawnGemArc(startX, this.floorY - 110, 3);
        } else if (rand < 0.62) {
            // Ceiling Stalactite cluster hanging down
            const count = Math.random() < 0.5 ? 1 : 2;
            for (let i = 0; i < count; i++) {
                const height = Math.floor(Math.random() * 30) + 45;
                this.obstacles.push({
            ...
            this.spawnGemArc(startX, this.ceilY + 80, 3);
        } else if (rand < 0.82) {
            // Dual Cave Pinch: Floor Stalagmite + Ceiling Stalactite
            this.obstacles.push({
                type: 'STALAGMITE',
                x: startX,
                y: this.floorY - 60,
            ...
            this.spawnRelic(startX + 65, 300);
        } else if (rand < 0.92 && this.level >= 2) {
            ...
            this.spawnGemArc(startX, 300, 3);
        } else {
            // Swinging Crystal Pendulum Trap (Higher levels)
            this.obstacles.push({
                type: 'PENDULUM',
                x: startX,
                y: 300,
