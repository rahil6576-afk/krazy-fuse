...
let baseSpeed = 4.8; // Fast, energetic arcade runner speed
...
function gameOver(customReason) {
    ...
    const isNewRecord = finalTime > bestSurvivalTime;
    if (isNewRecord) {
...
function updateSpawner() {
    ...
    if (spawnTimer <= 0) {
        ...
        const interval = Math.max(36, Math.floor((460 / gameSpeed) - (survivalTime * 0.1)));
        spawnTimer = interval;
...
function updateGame() {
    ...
    // Gradual speed progression over time and distance (Fast, exciting arcade pace)
    const timeSpeedBonus = survivalTime * 0.035; 
    const distSpeedBonus = distance * 0.0008;   
    const currentBaseSpeed = baseSpeed + Math.min(4.2, timeSpeedBonus + distSpeedBonus);

    // Coffee & Dash speed boost multipliers
    const speedBoost = player.coffeeTimer > 0 ? 1.38 : (player.isDashing ? 1.5 : 1.0);
